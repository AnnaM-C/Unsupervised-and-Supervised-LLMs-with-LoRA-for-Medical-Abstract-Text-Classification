...
plt.legend()
plt.savefig(f'graphs/ELECTRA_nolora_loss.png')  # Save the plot
plt.close()

train_curves = pd.concat([selected_loss, train_eval], axis=1)

train_curves.to_csv(f'{csv_dir}/ELECTRA_nolora_train_curves.csv', index=False)
...
train_metrics.to_csv(f'{csv_dir}/ELECTRA_nolora_train_metrics.csv', index=False)
...
df.to_csv(f'{csv_dir}/ELECTRA_nolora_Output.csv', index=True)
...
test_preds_labels.to_csv(f'{csv_dir}/ELECTRA_nolora_preds_labels.csv', index = False)
...
cmdispl.plot(ax=ax)
plt.savefig(f'confusionmatrix/ELECTRA_nolora_confusion_matrix.png')
plt.close()
