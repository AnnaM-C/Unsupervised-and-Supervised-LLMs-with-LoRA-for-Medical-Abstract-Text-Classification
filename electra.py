...
training_args = TrainingArguments(
    ...
    learning_rate=0.000258974093456998,
...
plt.legend()
plt.savefig(f'graphs/Electra_loss{rank}.png')
plt.close()

train_curves = pd.concat([selected_loss, train_eval], axis=1)

train_curves.to_csv(f'{csv_dir}/ELECTRA_train_curves_rank{rank}.csv', index=False)
...
train_metrics.to_csv(f'{csv_dir}/Electra_train_metrics_rank{rank}.csv', index=False)
...
df.to_csv(f'{csv_dir}/ElectraOutput_rank{rank}.csv', index=True)
...
test_preds_labels.to_csv(f'{csv_dir}/Electra_preds_labels_rank{rank}.csv', index = False)
...
cmdispl.plot(ax=ax)
plt.savefig(f'confusionmatrix/ElectraOutput_confusion_matrix_rank{rank}.png')
plt.close()